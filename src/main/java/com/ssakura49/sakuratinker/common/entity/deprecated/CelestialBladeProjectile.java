...
public class CelestialBladeProjectile extends Projectile implements IEntityAdditionalSpawnData, IProjectileCritical, IProjectileDamage, ItemSupplier, IProjectileRange {
    ...
    public static final EntityDataAccessor<Integer> DATA_COLOR = SynchedEntityData.defineId(CelestialBladeProjectile.class, EntityDataSerializers.INT);
    public static final EntityDataAccessor<Boolean> DATA_BACK = SynchedEntityData.defineId(CelestialBladeProjectile.class, EntityDataSerializers.BOOLEAN);

    public static final EntityDataAccessor<Float> DATA_PITCH = SynchedEntityData.defineId(CelestialBladeProjectile.class, EntityDataSerializers.FLOAT);
    public static final EntityDataAccessor<Float> DATA_YAW = SynchedEntityData.defineId(CelestialBladeProjectile.class, EntityDataSerializers.FLOAT);
    public static final EntityDataAccessor<Float> DATA_ROLL = SynchedEntityData.defineId(CelestialBladeProjectile.class, EntityDataSerializers.FLOAT);
    public static final EntityDataAccessor<Integer> DATA_AGE = SynchedEntityData.defineId(CelestialBladeProjectile.class, EntityDataSerializers.INT);

    public static final EntityDataAccessor<CelestialBladeConfig> DATA_CONFIG = SynchedEntityData.defineId(CelestialBladeProjectile.class, STEntityDataSerializers.BLADE_CONFIG.get());
    ...
    private Vec3 ellipseLongAxis = Vec3.ZERO;//长轴方向向量
    private Vec3 ellipseShortAxis = Vec3.ZERO;//短轴方向向量
    ...
    public float prevRoll, renderRoll;
    ...
    protected int getRandomColor() {
        ...
        return lerpColor(colorFrom, colorTo, this.colorProgress );
    }

    public Vec3 getPositionAt(float progress) {
        CelestialBladeConfig cfg = this.entityData.get(DATA_CONFIG);
        Entity owner = getOwner();
        if (owner == null) return this.position();
        double angle = progress * Math.PI * 2;
        return owner.getEyePosition()
                .add(ellipseLongAxis.scale(Math.cos(angle) * cfg.longAxis()))
                .add(ellipseShortAxis.scale(Math.sin(angle) * cfg.shortAxis()));
    }
    ...
    public CelestialBladeProjectile(Level level, Player player, float baseDamage, ToolStack tool, InteractionHand hand) {
        ...
        this.entityData.set(DATA_PITCH, -player.getXRot());
        this.entityData.set(DATA_YAW, player.getYHeadRot()+90);

        float randomRoll = player.getRandom().nextFloat()*360f;
        this.entityData.set(DATA_ROLL,randomRoll);
        ...
        if (Math.abs(ellipseLongAxis.dot(up)) > 0.95) {
            up = new Vec3(1, 0, 0); //防止长轴直上直下时错误
        }
        Vec3 baseShortAxis = ellipseLongAxis.cross(up).normalize();

        float rollRad = (float) Math.toRadians(randomRoll);
        this.ellipseShortAxis = baseShortAxis.scale(Math.cos(rollRad))
                .add(ellipseLongAxis.cross(baseShortAxis).scale(Math.sin(rollRad)))
                .normalize();

        this.shortAxisLength = 2.0f + random.nextFloat() * 3.0f;
//        //随机短轴
...
//        this.shortAxisLength = Math.max(3.0f, 1.5f + random.nextFloat() * 2.0f);//随机长度
    ...
    public void tick() {
        ...
        if (level().isClientSide) {
            this.prevRoll = this.renderRoll;
            this.renderRoll = this.entityData.get(DATA_ROLL);

            this.tickCount++; // 别忘了增加客户端计数
            return;
        }

        if (!level().isClientSide) {
            float progress = (float)tickCount / cfg.lifespan();
            this.setPos(getPositionAt(progress));
    ...
    protected void defineSynchedData() {
        ...
        this.entityData.define(DATA_CONFIG, new CelestialBladeConfig(STItems.blade_convergence.get().builtInRegistryHolder(), 0xFFFFFF, 10, 3, 1.0, 20, 0.5));
        this.entityData.define(DATA_PITCH, 0f);
        this.entityData.define(DATA_YAW, 0f);
        this.entityData.define(DATA_ROLL, 0f);
